...
RSpec.describe Users::Tabs::Component, type: :component do
  let(:expect_mandatory_elements) do
    expect(rendered_content).to include('<turbo-frame data-turbo-action="advance" id="user-tabs" target="user-content">')
    ...
    expect(rendered_content).to include('<main class="w-2/3">')
