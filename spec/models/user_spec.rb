...
RSpec.describe User, type: :model do
  ...
  describe 'associations' do
    ...
    it { should have_many(:following_relations).class_name('Following').with_foreign_key('user_id') }
    ...
    it { should have_many(:followed_relations).class_name('Following').with_foreign_key('followed_id') }
    it { should have_many(:followers).through(:followed_relations).source(:following) }
  end

  it 'is valid with valid attributes' do
    user = User.new(username: 'hasan', fullname: 'Hasan Ozovali')
    user.update(email: 'hasan@email.com', password: 'aaaaaa', password_confirmation: 'aaaaaa')
