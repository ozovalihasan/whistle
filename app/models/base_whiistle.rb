...
class BaseWhiistle < ApplicationRecord
  ...
  def set_quoted_whiistle_id
    return unless quoted_whiistle_url

    path = URI.extract(quoted_whiistle_url).first
    all_parts = Rails.application.routes.recognize_path(path)
    return unless all_parts[:controller] == 'whiistles' && all_parts[:action] == 'show'

    self.quoted_whiistle_id = all_parts[:id]
  end

  def set_parent_id
    self.parent_id = (base_whiistle_id if base_whiistle_id && ([Reply, Flood].include? self.class))
  end
end
